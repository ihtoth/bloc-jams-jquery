...
//This should technically be in a "helper section"
  prettyTime (timeInSeconds){
    const s = timeInSeconds
    return `${Math.floor(s/60)%60}:${Math.floor(s%60)< 10 ?"0"+ Math.floor(s%60) : Math.floor(s%60)}`
    //when seconds are less than 10, want to display 0:01 instead of 0:1 for example.
  }
}
